...
for(n in unique(model_probes$`Gene Symbol`) ){
  ...
  probes[n] <- names(cors_tmp)[which(abs(cors_tmp) == max(abs(cors_tmp)) )]
  # if(logFCs[n,"logFC"] >0 ) {
...
```{r}
  ...
  test_res$obs <- Metadata_Training[rownames(X),]  %>% pull(var="response")
  g <- ggplot(test_res, aes(m=nonresponder, d=obs)) + #factor(obs, levels = c("no", "yes"))
    geom_roc(n.cuts=0) + 
    coord_equal() +
    style_roc()
  auc_for_plot <- paste("AUC =", format(round((calc_auc(g))$AUC, 2), nsmall=2))
  g = g + annotate("text", x=0.75, y=0.25, label=auc_for_plot) + ggtitle("Random Forest ROC")
  print(g)
  pdf("/data/pangelin/HUG/Thibaud/RC_GEOMX/analysis/results/Domingo_sig_4_6_0.05_RF_ROC.pdf")
  print(g)
  dev.off()
