...
params:
  ...
  seed: 26326
  save.model: '/export/big/sean/models/cv3_modeling_extra_fix_6datasets_sig_4_6_0.05.Rds'
  gene.list.file: "/export/big/sean/RCproject/gene_lists.rds"
...
---
  ...
  ```
lapply(c("Ago_2016", "g1", "Ghad_2005", "Gim_2016", "Kim_2007", "Park_2020", "RSS", "Wat_2006", "sig_4_6_0.05"), function(x) { rmarkdown::render("/export/big/sean/RCproject_code/sean/modeling_extra_final_copy.Rmd", output_file=paste0("/export/big/sean/reports/CV3.modeling_extra_final.",x,".report.html"), params = list(gene.list = x, title = paste0("public signature model CV, gene list ", x))) })
```
...
```
## Adding quantile normalization assay and comparing count distributionst between assays
...
if (params$assay == 'scale') {
  assay_to_use <- 3
  assayname_to_use <- "scalelogcounts"
} else if (params$assay == 'quantile') {  
  assay_to_use <- 4
...
plotRLExpr(spe, ordannots = "batch", assay = 1, color = batch)
print('log counts')
plotRLExpr(spe, ordannots = "batch", assay = 2, color = batch)
print('scaled counts')
plotRLExpr(spe, ordannots = "batch", assay = 3, color = batch)
print('quantile normalized counts')
plotRLExpr(spe, ordannots = "batch", assay = 4, color = batch)
