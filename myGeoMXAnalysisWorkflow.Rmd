...
Data <- readRDS("/data/pangelin/HUG/Thibaud/RC_GEOMX/data/bothCohorts_raw.Rds")
...
design <- model.matrix(~0 + Type + SlideName, data = colData(spe))
...
rownames(v) <- rowData(spe)[rownames(v),"SystematicName"]
...
dovissE(fry_res_sig, de_genes_toptable_BvT, topN = 9, title = "Post vs. Pre" )
```
...
```{r}
samples_subset <- colnames(spe_tmm)[colData(spe_tmm)$segment %in%  c("CK-", "CK+")]
...
```{r}
spe_sub$segment <- sub("CK\\+", "tumor",spe_sub$segment)
spe_sub$segment <- sub("CK\\-", "stroma",spe_sub$segment)
