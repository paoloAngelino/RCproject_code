---
title: "Analysing bulk transcriptomics data using standR, limma and vissE, 9 datasets, scaled data"
author:
  - name: Paolo Angelino
    affiliation:
...
params:
  ...
  usescaled: TRUE
  save_de_results : "/data/pangelin/HUG/Thibaud/RC_GEOMX/analysis/results/DE_9Datasets_scaled_2.rds"
  cv: 10
  seed: 13947
  ensemble_method: "intersect"
  pval_threshold: 0.05
---
...
```{r}
spe <- spe[, spe$batch %in% c("GSE150082", "GSE94104", "GSE133057", "GSE93375", "GSE209746", "GSE145037", "GSE190826",  "GSE45404_GPL1", "GSE45404_GPL2")]
#spe <- spe[, spe$batch %in% c("GSE209746")] #, "GSE150082", "GSE94104", "GSE133057", "GSE93375", "GSE145037"
```
...
run_limma <- function(lspe, lassay_to_use, verbose = TRUE, p.value = 0.05, lfc = 0.0){

  design <- model.matrix(~0 + Response + batch, data = colData(lspe))
...
```{r}
if(params$cv == 1) {
  de_results <- run_limma(spe, assay_to_use)
  de_results$gene_name <- rownames(de_results)
  de_genes_toptable <- de_results[de_results$P.Value <= params$pval_threshold, ]
} else {
  set.seed(params$seed)
  my_folds <- caret::createFolds(spe$Response, k = params$cv)
  
  # structure of "my_fold"
  my_folds %>% glimpse
  # distribution of class is preserved in each fold
  my_folds %>% 
    map_df(~prop.table(table(spe$Response[.])))

  my_combn <- combn(names(my_folds),params$cv*0.8)
  
  de_all <- list()
  for(n in 1:ncol(my_combn)){
    ids = unlist(my_folds[my_combn[,n]])
    de_results <- run_limma(spe[,ids], assay_to_use)
    de_all[[n]] <- de_results
  }
  
  getMedians <- function(my_de_all, i){
    vals <- lapply(my_de_all, function(x){
      x[,i]
    })
  ...
  getPval <- function(my_de_all, i, method){
    vals <- lapply(my_de_all, function(x){
      x[,i]
    })
    vals <- do.call(cbind, vals)
  ...
  names(de_results)
  de_results <- data.frame(logFC = getMedians(de_all,1), logFC_sd = getSds(de_all,1), 
                           AveExpr = getMedians(de_all,2), AveExpr_sd = getSds(de_all,2), 
                           t = getMedians(de_all, 3),
                           P.Value = getPval(de_all,4,method = params$ensemble_method),
                           P.Value_union = getPval(de_all,4,method = "union"),
                           P.Value_intersect = getPval(de_all,4,method = "intersect"),
                           P.Value_avg = getMedians(de_all,4), P.value_sd = getSds(de_all,4), 
                           adj.P.Val = getPval(de_all,5,method = params$ensemble_method), 
                           B = getMedians(de_all,5),
                           row.names = rownames(de_all[[1]]))
  de_results <- de_results[order(de_results$P.Value, decreasing = F),]
  de_results$gene_name <- rownames(de_results)
  de_genes_toptable <- de_results[de_results$P.Value <= params$pval_threshold, ] 
}
...
print(plotVolcano(de_results, "Yes_vs_NO"))
```
...
```{r fig.width=10, fig.height=9}
  p <- de_results %>% 
    mutate(DE = ifelse(logFC > 0 & P.Value <0.1, "UP", 
                       ifelse(logFC <0 & P.Value <0.1, "DOWN", "NOT DE"))) %>%
...
results <- de_genes_toptable
results$gene_name <- rownames(results)
GOI <- unique(subset(results, `P.Value` < 0.05 & abs(logFC) > 0)$gene_name)
